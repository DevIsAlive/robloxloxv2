...
export async function getRobloxUserHeadshot(username: string): Promise<RobloxUserResponse> {
  try {
    ...
    if (!userSearchResponse.ok) {
      console.error("User search failed:", userSearchResponse.status, userSearchResponse.statusText)
    ...
    if (!headshotResponse.ok) {
      console.error("Headshot fetch failed:", headshotResponse.status, headshotResponse.statusText)
    ...
    console.log("Headshot response:", headshotData)
