...
import { Box, HandIcon as HandClick, Gift } from "lucide-react"
      ...
      <div className="flex flex-col items-center justify-center flex-1 w-full max-w-md">
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} className="text-center w-full">
          ...
          >
            ...
            >
              <Box className="w-20 h-20 text-blue-500 mx-auto mb-3" />
            </motion.div>
            <h2 className="text-4xl font-black text-blue-700 mb-2 drop-shadow-lg">{t.pickYourPrize}</h2>
            <p className="text-blue-600 font-bold text-xl drop-shadow-sm">{t.chooseBox}</p>
          </motion.div>
            ...
            {[0, 1, 2].map((i) => (
              <motion.button
                ...
                className={`
                  ...
                  ${
                    selectedBox === i
                      ? "bg-gradient-to-br from-yellow-300 to-yellow-500 border-yellow-600 legendary-glow shadow-2xl"
                      : "bg-gradient-to-br from-blue-500 to-blue-700 border-blue-800 shadow-xl"
                  }
                  ${selectedBox !== null && selectedBox !== i ? "opacity-50 grayscale" : ""}
                `}
                whileHover={{
                  scale: selectedBox === null ? 1.08 : 1,
                  boxShadow: selectedBox === null ? "0 15px 30px rgba(0,0,0,0.4)" : undefined,
                  y: selectedBox === null ? -10 : 0,
                }}
